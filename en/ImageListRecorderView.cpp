...
CImageListRecorderView::CImageListRecorderView()
{
	...
	if(hModule==NULL){
		MessageBox("Cannot find the xvidcore.dll file, please add the file to the same directioy of the exe file.");
		xvid_global=NULL;
		xvid_decore=NULL;
		xvid_encore=NULL;
	...
	if(xvid_global==NULL || xvid_decore==NULL || xvid_encore==NULL){
		MessageBox("Not valid xvid file.");
	}
	if(xvid_global(NULL,XVID_GBL_INIT,&xvidGblInit,NULL)<0){
		MessageBox("Xvid library init failed.");
	}
	if(!myMp3EncLoadDllFile("mp3enc.dll")){
		MessageBox("Cannot find the correct file mp3enc.dll.");
	}
...
void CImageListRecorderView::OnDraw(CDC* pDC)
{
	...
	ASSERT_VALID(pDoc);
	if(cimage.GetWidth()>10 && cimage.GetHeight()>10){
...
UINT RecordThread(LPVOID pParam) 
{
	...
	if(pView->fileType==5){
		if(xvid_encore(NULL,XVID_ENC_CREATE,&(pView->avi0),NULL)<0){
			AfxMessageBox("Xvid init failed.",MB_OK,MB_ICONSTOP);
			pView->recordMode=false;
			return 0;
		}
		pView->avi0Handle=pView->avi0.handle;
	...
	if(pView->fileType==5){
		pView->aviFile=fopen((LPTSTR)(LPCTSTR)(pView->fileName0),"wb");
		if(pView->aviFile==NULL){
			AfxMessageBox("Creating file failed.",MB_OK,MB_ICONSTOP);
			pView->recordMode=false;
			return 0;
		}
		...
		if(fclose(pView->aviFile)!=0){
			AfxMessageBox("Initing file failed.",MB_OK,MB_ICONSTOP);
			pView->aviFile=NULL;
			pView->recordMode=false;
			return 0;
		}
...
void CImageListRecorderView::OnFileNew() 
{
	...
	CFileDialog dlg2(FALSE,NULL,NULL,NULL,"BMP List(*.bmp)|*.bmp|JPG List(*.jpg;jpeg)|*.jpg;*.jpeg|PNG List(*.png)|*.png|GIF List(*.gif)|*.gif|XVID AVI File(*.avi)|*.avi||");//Raw AVI File(*.avi)|*.avi||");
	int nStructSize = 0;
	...
	if(dlg.DoModal()==IDOK && dlg2.DoModal()==IDOK){
		...
		if(fp!=NULL){
			fclose(fp);
			fp=NULL;
			if(MessageBox(fileName0+"Override the existing file?","Warning",MB_YESNO|MB_ICONWARNING)!=IDYES){
				return;
			}
		}
		...
		if(cimage.Create(imageWidth,imageHeight,24)!=TRUE){
			MessageBox("Failed while creating bitmaps.");
			return;
		}
		if(!cimage2.IsNull()){
			cimage2.Destroy();
...
void CImageListRecorderView::OnUpdateTingZhiLuZhi(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	//if(recordMode)
		pCmdUI->SetText("&Stop Rerording\tEsc");
	//else
	//	pCmdUI->SetText("&Revert\tEsc");
}
