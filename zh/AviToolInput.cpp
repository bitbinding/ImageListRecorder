...
void CAviToolInput::OnButton1() 
{
	...
	CFileDialog dlg(TRUE,NULL,NULL,OFN_ALLOWMULTISELECT,"图片序列文件(*.bmp;*.jpg;*.jpeg;*.png;*.gif)|*.bmp;*.jpg;*.jpeg;*.png;*.gif||");
	int nStructSize = 0;
	...
	if(result==IDOK){
		...
		if(count>0){
			...
			CString cstr;
			cstr.Format("基本：%s",(LPTSTR)(LPCTSTR)imageFilePathNameList[0]);
			m_s1.SetWindowText(cstr);
			cstr.Format("缩放：%s",(LPTSTR)(LPCTSTR)imageFilePathNameList[count-1]);
...
{
	// TODO: Add your control notification handler code here
	CFileDialog dlg(TRUE,NULL,NULL,NULL,"avi文件(*.avi)|*.avi||");
	int nStructSize = 0;
    DWORD dwVersion, dwWindowsMajorVersion, dwWindwosMinorVersion;
    dwVersion = GetVersion();
    dwWindowsMajorVersion = (DWORD)(LOBYTE(LOWORD(dwVersion)));
	...
	if(dlg.DoModal()==IDOK){		
		...
		cstr.Format("基本：%s",(LPTSTR)(LPCTSTR)fnamea0);
		fnamea.Format(_T("%s"),fnamea0);
		m_s1.SetWindowText(cstr);
		m_s2.SetWindowText("缩放");
...
void CAviToolInput::OnButton3() 
{
	...
	if(title=="视频修复"){
		CFileDialog dlg(TRUE,NULL,NULL,NULL,"被修复的avi副本文件(*.avi)|*.avi||");
		dlg.m_ofn.lStructSize = nStructSize; //------用来检测目前的操作系统的版本信息
		if(dlg.DoModal()!=IDOK){
			return;
		}
		if(MessageBox("修复前最好先作好备份，并且在修复中将会修改当前文件，确实要继续吗？","注意",MB_YESNO)!=IDYES){
			return;
		}
		aviFixFileName=dlg.GetPathName();
		AfxBeginThread(aviFix,this);
	}else if(title=="视频混流"){
		MessageBox("音视频混流功能仅供测试（无损混流），请确保视频是由本程序生成的无声视频(每100帧一个关键帧)，与MP3的时长大致一致，并且MP3格式为MPEG1-Layer3，位率为128kbps,采样频率为44.1khz的音频（即帧长为417字节的固定码率音频）。");
		CFileDialog dlg1(TRUE,NULL,NULL,NULL,"原avi视频文件(*.avi)|*.avi||");
		CFileDialog dlg2(TRUE,NULL,NULL,NULL,"原mp3音频文件(*.mp3)|*.mp3||");
		CFileDialog dlg3(FALSE,NULL,NULL,OFN_OVERWRITEPROMPT,"目标avi视频文件(*.avi)|*.avi||");
		if(dlg1.DoModal()==IDOK && dlg2.DoModal()==IDOK && dlg3.DoModal()==IDOK){
			SetWindowText("正在混流，请稍候。");
			CString muxedAviName=dlg3.GetPathName();
			CString muxedAviNameRear=muxedAviName.Right(4);
			muxedAviNameRear.MakeLower();
			if(muxedAviNameRear!=".avi"){
				muxedAviName+=".avi";
			}
			aviMux((LPTSTR)(LPCTSTR)dlg1.GetPathName(),(LPTSTR)(LPCTSTR)dlg2.GetPathName(),
				(LPTSTR)(LPCTSTR)muxedAviName);
			SetWindowText("混流完成。");
		}
	}
}
...
UINT CAviToolInput::aviFix(LPVOID pParam){
	...
	if(movipos>=8 && movipos<=leng){
		...
		pWnd->MessageBox("修复失败，视频流信息开始处在错误的位置。");
		pWnd->SetWindowText(windowText0);
		pWnd->isFixing=false;
		return 0;
	}
	...
	for(i=0;i<idx1Leng;i++){
		...
		cstr+=cstr2;
		if(i%100==0){
			cstr.Format("正在计算记录的长度:%d/%d...",i,idx1Leng);
			pWnd->SetWindowText(cstr);
		}
	}
	...
	while(true){
		...
		if(j<0){
			...
			if(j==0){
				...
				j=0;
				if(i%100==0){
					cstr.Format("正在重新生成视频索引:%d/%d...",i,idx1Leng);
					pWnd->SetWindowText(cstr);
				}
				i++;
